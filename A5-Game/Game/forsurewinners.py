...
class MyPlayer(Player):
	...
	def reset(self, player_id, max_players, width, height):
		self.player_name = "forsurewinners"
	...
	def opponent_is_closer(self, status, gold_pos, my_steps):
		# if another player is 3 tiles closer to the gold pot, the bot doesn't race
		for other in status.others:
			if other is None:
				continue
			opp_dist = self.chebyshev((other.x, other.y), gold_pos)
			if opp_dist < my_steps - 3 :
				return True
		return False
	

	def next_tile_to_explore (self, start, gold_pos, remaining):
		best_dir = None
		...
		while queue:
			...
			# if the tile can't be reached in the remaining time until gold moves, skip it
			if dist >= remaining - 2:
				continue
			...
			for d in D:
				...
				if self.known_map[nx, ny].status == TileStatus.Unknown:
					edge_tile = True
				elif self.is_safe_tile(nx, ny, allow_unknown=False) and (nx, ny) not in visited:
					visited.add((nx, ny))
					parent[(nx, ny)] = cur
					move_to[(nx, ny)] = move_to[cur] if parent[cur] is not None else d
					queue.append((nx, ny))

			if edge_tile and cur != start:
				gold_dist = self.chebyshev(cur, gold_pos)
				recent_penalty = 5.0 if cur in self.recent_positions else 0.0
				score = dist + 0.5 * gold_dist + recent_penalty
				if score < best_score:
					best_score = score
					n = cur
					while parent[parent[n]] is not None:
						n = parent[n]
					best_dir = move_to[n]

		return [best_dir] if best_dir is not None else []

	def step_toward(self, start, target):
		
		# step toward defined target
		...
		while queue:
			...
			for d in D:
				...
				if nxt in visited:
					continue
				if not self.is_safe_tile(nxt[0], nxt[1], allow_unknown=False):
					continue

				visited.add(nxt)
				parent[nxt] = cur
				move_to[nxt] = move_to[cur] if parent[cur] is not None else d
	...
	def move(self, status):
		...
		if status.goldPots:
			...
			if path:
				...
				if self.opponent_is_closer(status, gold, steps):
				
					center = (self.width // 2, self.height // 2)
					return self.step_toward(start, center)
			...
			# explore edge_tiles, else move in the direction of the center 
			if status.goldPots:
				gold_pos = next(iter(status.goldPots.keys()))
				edge_tile = self.next_tile_to_explore(start, gold_pos, status.goldPotRemainingRounds)
				if edge_tile:
					return edge_tile
				center = (self.width // 2, self.height // 2)
				return self.step_toward(start, center)
