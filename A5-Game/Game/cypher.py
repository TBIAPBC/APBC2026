...
class cypher(Player):
    ...
    def _update_map(self, status):
        ...
        for other in status.others: 
            if other is None:
                    continue
            if other.player == status.player - 1:
                x,y = (other.x, other.y)
    ...
    def _should_wait(self, status, distance, pot_value):
        ...
        move_cost = cypher._movement_cost(min(max_moves, distance))
    ...
    def _best_gold_target(self, status, curpos):
        ...
        for gLoc, gold_value in status.goldPots.items():
            ...
            if self._movement_cost(my_distance) > status.gold:
                continue
            ...
            for other_status in status.others:
                if other_status is None:
                    continue
                #print(f'player:{other_status.player}')
                #print(f' who am i :{status.player}')
                if other_status.player == status.player - 1:
    ...
    def _num_moves_to_make(self, status, distance, pot_value):
        ...
        while n > 0 and cypher._movement_cost(n) >= pot_value:
            n -= 1
            
        if self._movement_cost(distance) < max_affordable and self._movement_cost(distance) <= pot_value:
            n = distance
    ...
    def move(self, status):
        ...
        if numMoves == 0:
            return self._fallback_move(status, curpos)
        #print(self._as_directions(curpos, bestpath[:numMoves]))
        return self._as_directions(curpos,bestpath[:numMoves])
    ...
    def _fallback_move(self, status, curpos):
        ...
        if distance > status.goldPotRemainingRounds:
            ...
            if center_path:
                ...
                #print('i want to move towards the gold')
