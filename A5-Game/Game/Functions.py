...
#still todo is collision and unkown tiles
...
class MyPlayer(Player):
    ...
    #better to see new tiles then visited ones so save visited??
		self.visit_count = {}
	...
	def breadth_first_search(self, start, goal, status):
		...
		return []
#helper iterating every possible pot also calling the shortest path and deciding on score so path lenthg and amount when equal
...
#list of possible moves, bonds and wall check and then score(best if unknown) so no walls, and best unseen ziles
...
#finds gold pots in sight, sets preference based on distance, size
		if status.goldPots:
